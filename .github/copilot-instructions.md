...
- **Engine-first design**: React is a pure view layer consuming immutable snapshots from `StateManager`; simulation ticks own all writes. The frame spine runs `input → streaming → jobsFlush → simulation → renderSync → draw`.
...
## Key conventions
- Mutations only during the fixed 60 Hz simulation tick; use the command queue and simulation systems for changes. React and render loops must stay read-only.
...
- For new systems, follow the modular pattern: create a system class with `update`/`handleCommand`, register it via `AureusWorld`, and ensure it flags chunk or render dirty states as needed.
